...
const PaymentAPI = {
  ...
  // Memanggil Edge Function 'create-payment' — generik utk semua jenis produk
  async buatCheckoutProduk(tipeProduk, produkId, gateway) {
    ...
    const res = await fetch(`${window.APP_CONFIG.SUPABASE_URL}/functions/v1/create-payment`, {
      ...
      body: JSON.stringify({ tipe_produk: tipeProduk, produk_id: produkId, gateway }),
    });
  ...
  // Alias lama, tetap dipakai halaman Langganan (paket langganan/subscription)
  async buatCheckout(paketId, gateway) {
    return this.buatCheckoutProduk("langganan", paketId, gateway);
  },
};
